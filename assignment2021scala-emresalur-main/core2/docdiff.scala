...
def occurrences(xs: List[String]): Map[String, Int] = {
    xs.groupBy(identity).map(lst => (lst._1, lst._2.size))
    }
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    val products = for (i <- occurrences(lst1).keys.toList;
        j <- occurrences(lst2).keys.toList;
        if i==j) yield occurrences(lst1)(i) * occurrences(lst2)(j)
    products.sum
   }
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    prod(lst1,lst2).toFloat/max(prod(lst1,lst1),prod(lst2,lst2))
    }
